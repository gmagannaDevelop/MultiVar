...
D'après le graphique $\left(x_i,\hat{\varepsilon_i}\right)$, nous remarquons que
...
reg3<-lm(O3~T12,data=df[df$T12>21,])
summary(reg3)
```
Dans le premier modèle (1), nous remarquons que le coefficient $\beta_2^\star$
n'est pas significatif. En effet, la $p-\text{value}$ est de 0.136 donc il n'y a aucune significativité
même au seuil de $\alpha = 10\%$. Concernant le modèle (2), le coefficient $\beta_2^\ast$
est très significatif avec une $p-\text{value} \approx 0$. Nous remarquons également un
...
car la concentration en O3 n'est pas expliquée par la température en deçà de 20 degrés celcius.

Question 8

Supposons que la température soit expliquée par elle-même au carré, alors le logarithme
de la concentration en O3 dans l'air peut exploser à condition que la température
soit très élevée car au carré, cela augmente très vite. Ce qui explique que, comme nous
raisonnons au carré, la pente de la courbe est nulle en deçà d'une certaine température,
et que ça devient concentré en O3 au dessus. Je pense que cette démonstration est fallacieuse.
Mais il est tellement illégal qu'aucune peine n'est possible face à une telle situation.
Aucun loi ne peut tolérer cela, mais aucune punition, même au carré, pourra corriger
